...
/*   Updated: 2022/01/14 16:10:26 by dkramer       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	getpathandoptions(char **argv, int argint, t_pipex *pipex, char **newenv)
{
...
void	parentprocess(int *pipefd, char **argv, char **newenv, t_pipex *pipex)
{
	...
	if (access(argv[4], 0000) == 0)
		if (unlink(argv[4]) == -1)
			exit(EXIT_FAILURE);
	getpathandoptions(argv, 2, pipex, newenv);
	printf("%s\n", pipex->path);
...
void	childprocess(int *pipefd, char **argv, char **newenv, t_pipex *pipex)
{
	if (close(pipefd[1]) == -1)
		exit(EXIT_FAILURE);
	getpathandoptions(argv, 3, pipex, newenv);
	pipex->args = malloc(sizeof(char *) * 4);
	if (!pipex->args)
		exit(EXIT_FAILURE);
	pipex->args[0] = pipex->path;
